...
output_shp_study_area <- here("output_data/grid_7km/Amazon_grid.shp")
...
if (output_shp_current %>% file_exists() == F) {
  grid_current <- grid_study_area %>% 
    add_raster(folder_current_rasters, raster_vars, scale=T) 
...
if (!all(output_shp_future %>% file_exists())) {
  for (i in 1:length(scenarios)) {
      ...
      grid_future <- grid_study_area %>% 
       add_raster(folder_future_rasters, future_var_names, scenarios[i], scale=T) 
...
spp_output <- here("output_data/spp_pa_7km.shp")
```
...
```{r}
indep_test <- paste0(dirname(spp_data), '/spp_data.csv')

occ_indep_shp <- indep_test %>% 
  occurrences_to_shapefile(spp_names[2], grid_study_area)


library(pROC)
auc(roc(ensembles$current$current_pa_mean, rep(1,nrow(occ_indep_shp$species)))
```
